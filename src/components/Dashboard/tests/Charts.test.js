...
describe('EconomicChart component', () => {
  it('should render the loading message when fetching data', async () => {
    render(<EconomicChart token="dummyToken" />);
  ...
  it('should render the charts and select box when data is fetched', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Budget Allocation for the Current Period Compared to Cumulative Budget')).toBeInTheDocument();
      expect(screen.getByText('Budget Allocation for the Current Period')).toBeInTheDocument();
      expect(screen.getByText('Budget Allocation for the Current Period Compared to Cumulative Budget on Area Chart')).toBeInTheDocument();
